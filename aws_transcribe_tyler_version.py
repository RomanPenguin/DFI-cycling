...
#def main(argv):
    ...
    #comment block: transcription via command line 
    # try:
        ...
        for item in items:
            ...
            # else, keep going until you hit a punctuation
                ...
                if (
                    ...
                    end_time = float(items[i - 1]["end_time"]) if i - 1 >= 0 else float(items[0]["end_time"])
            ...
            i = i + 1
        #once again testing alternative
        sentences_and_times.append(temp_start_time)
    ...
    with open(outputFilePath + "_sentences.csv", 'w') as f:
        #print(items, file=f)
        writer = csv.writer(f)
        #writer.writerow(sentences_and_times_1)
        total_points = len(sentences_and_times_1)
        i = 0
        while i < total_points:
            writer.writerow(sentences_and_times_1[i:i+4])
            i = i + 4
